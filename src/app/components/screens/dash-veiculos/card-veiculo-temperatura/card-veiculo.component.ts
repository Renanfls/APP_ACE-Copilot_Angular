...
@Component({
  selector: 'app-card-veiculo-temperatura',
  standalone: true, 
...
export class CardVeiculoTemperaturaComponent implements OnInit, OnDestroy {
  veiculos: any[] = [];
  ...
  ngOnInit() {
    console.log('🔄 Initializing temperature component');
    this.componentRegistry.registerComponent(this);
    this.loadVehicles();
  }

  ngOnDestroy() {
    console.log('🔄 Destroying temperature component');
    ...
    setTimeout(() => {
      this.http.get<any>('assets/mocks/veiculos_mock.json').subscribe({
        next: (data) => {
          ...
          // Ordenar veículos pela prioridade do atributo de temperatura
  ...
  private startAutoSlide() {
    console.log('🎠 Starting auto slide for temperature component');
  ...
  getAttributePriority(veiculo: any): number {
    ...
    // Considera apenas o atributo de temperatura (índice 0)
    return this.getColorPriority(veiculo.atributos[0].cor);
  }
}