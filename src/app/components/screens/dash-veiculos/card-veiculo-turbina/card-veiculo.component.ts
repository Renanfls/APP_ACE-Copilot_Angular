...
@Component({
  selector: 'app-card-veiculo-turbina',
  standalone: true, 
...
export class CardVeiculoTurbinaComponent implements OnInit, OnDestroy {
  isLoading = true;
  veiculos: any[] = [];
  ...
  ngOnInit() {
    console.log('🔄 Initializing turbine component');
    this.componentRegistry.registerComponent(this);
    this.loadVehicles();
  }

  ngOnDestroy() {
    console.log('🔄 Destroying turbine component');
  ...
  private startAutoSlide() {
    console.log('🎠 Starting auto slide for turbine component');
    this.lastSlideShown = false;
    this.autoSlideInterval = setInterval(() => {
      if (this.currentSlide === this.vehicleGroups.length - 1) {
        console.log('🎠 Last slide reached in turbine component');
