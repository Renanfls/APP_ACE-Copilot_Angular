...
export class CardVeiculoArComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    console.log('🔄 Initializing air compressed component');
    this.componentRegistry.registerComponent(this);
    this.loadVehicles();
  }

  ngOnDestroy() {
    console.log('🔄 Destroying air compressed component');
  ...
  getAttributePriority(veiculo: any): number {
    ...
    // Considera apenas o atributo de ar comprimido (índice 4)
    return this.getColorPriority(veiculo.atributos[4].cor);
  }
