...
export class CardVeiculoPedalComponent implements OnInit, OnDestroy {
  isLoading = true;
  veiculos: any[] = [];
  ...
  ngOnInit() {
    console.log('🔄 Initializing pedal component');
    this.componentRegistry.registerComponent(this);
    this.loadVehicles();
  }

  ngOnDestroy() {
    console.log('🔄 Destroying pedal component');
  ...
  private startAutoSlide() {
    console.log('🎠 Starting auto slide for pedal component');
    this.lastSlideShown = false;
    this.autoSlideInterval = setInterval(() => {
      if (this.currentSlide === this.vehicleGroups.length - 1) {
        console.log('🎠 Last slide reached in pedal component');
